...
- For knowledge-interface with local `qdrant/qdrant:v1.9.2`, keep Rust `qdrant-client` on a compatible minor (for example `1.10.x`); large version gaps can break gRPC upserts even when connections succeed.
