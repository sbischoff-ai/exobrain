...
- If `MAIN_AGENT_USE_MOCK=true` or `OPENAI_API_KEY` is not set, embeddings are generated by a deterministic mock embedder for offline/agent development.
...
All graph writes now flow through a single application path (`KnowledgeApplication::ingest_graph_delta`) that:
