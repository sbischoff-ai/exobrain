...
Internal Memgraph timestamp triggers are ensured idempotently during startup via `CREATE TRIGGER ... IF NOT EXISTS`, so pre-existing triggers are treated as already-initialized state instead of a fatal error.
