...
- `QDRANT_ADDR` must point to the Qdrant gRPC endpoint (for example `http://localhost:6334`), not the REST dashboard port (`6333`).

Operational env vars used most often:

- `METASTORE_DSN`: Postgres DSN for canonical schema metadata.
- `MEMGRAPH_ADDR`: Memgraph Bolt endpoint used for graph writes/reads.
- `MEMGRAPH_DB`: Memgraph database name (defaults to `memgraph`).
- `QDRANT_ADDR`: Qdrant gRPC endpoint for `blocks`, `schema_node_types`, `schema_edge_types`.
- `EMBEDDING_USE_MOCK`: set `true` for deterministic offline embeddings.
- `MODEL_PROVIDER_BASE_URL` + `MODEL_PROVIDER_EMBEDDING_ALIAS`: required when mock embeddings are disabled.
...
Backfill runs before gRPC traffic is accepted, so node/edge type semantic lookup quality may be degraded until startup sync completes after a schema reset.
...
- **`FindEntityCandidates` returns empty/weak semantic matches right after reset**
  - Confirm Qdrant is reachable via `QDRANT_ADDR` and startup completed without schema/vector sync errors.
  - Ensure `blocks` has been populated by `UpsertGraphDelta` writes; semantic candidate scoring depends on block vectors.
- **`FindNodeTypeCandidates` / `FindEdgeTypeCandidates` return empty candidates**
  - Check that startup backfill finished and `schema_node_types` / `schema_edge_types` are populated.
  - If you changed schema types while the service was down, restart the service to force startup reconciliation.
- **Type-candidate ranking looks inconsistent across deploys**
  - Verify indexing and query paths still use the same canonical embedding templates (`entity type: ...` / `relationship type: ...`).
