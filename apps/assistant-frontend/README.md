...
- Knowledge content blocks use page-owned persistence: each block emits save requests, `KnowledgePage` calls `PATCH /api/knowledge/page/{page_id}` with `markdown_content`, prevents duplicate in-flight saves per block, commits markdown only on successful responses, restores last persisted content on failure, and announces save results through accessible status messaging.
