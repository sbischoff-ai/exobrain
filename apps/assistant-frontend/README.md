...
- Knowledge content blocks now persist edits server-side: each block emits save requests, `KnowledgePage` calls `PATCH /api/knowledge/page/{page_id}` with `content_blocks[].markdown_content`, prevents duplicate in-flight saves per block, updates rendered markdown only after successful responses, restores the last persisted markdown on failures, and announces success/error feedback through accessible status messaging.
