...
from app.worker.jobs.knowledge_update import (
    ...
    _build_step_five_comparison_prompt,
...
def test_step_five_comparison_prompt_compacts_candidate_contexts() -> None:
    prompt = _build_step_five_comparison_prompt(
        extracted_entity={"name": "Alice"},
        focused_markdown="x" * 9000,
        candidate_contexts=[
            {
                "entity": {"id": f"e{i}", "type_id": "node.person", "name": "Alice", "aliases": ["A"]},
                "blocks": [
                    {"id": "b1", "block_level": 0, "text": "t" * 1200},
                    {"id": "b2", "block_level": 1, "text": "u" * 1200},
                    {"id": "b3", "block_level": 2, "text": "v" * 1200},
                ],
            }
            for i in range(6)
        ],
    ...
    assert len(payload["candidate_contexts"]) == 4
    assert len(payload["extracted_entity_markdown"]) == 2000
