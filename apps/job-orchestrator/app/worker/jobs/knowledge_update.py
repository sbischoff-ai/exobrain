...
async def _run_step_five_detailed_comparison(
    ...
    for match_item in candidate_matching:
        ...
        if status == "matched" and isinstance(match_item.get("matched_entity_id"), str):
            ...
            prompt = _build_step_five_comparison_prompt(
                extracted_entity,
                focused_markdown,
                candidate_contexts,
                tight=True,
            ...
            logger.info(
                "knowledge.update step five comparison prompt built",
                extra={"candidate_count": len(candidate_contexts), "prompt_chars": len(prompt)},
            )
...
def _compact_entity_context_for_comparison(context: dict[str, object], *, tight: bool = False) -> dict[str, object]:
    core = context.get("entity") if isinstance(context.get("entity"), dict) else {}
    blocks = context.get("blocks") if isinstance(context.get("blocks"), list) else []
    max_blocks = 2 if tight else 5
    max_chars = 300 if tight else 800
...
def _build_step_five_comparison_prompt(
    extracted_entity: dict[str, object],
    focused_markdown: str,
    candidate_contexts: list[dict[str, object]],
    *,
    tight: bool = False,
) -> str:
    limited_contexts = candidate_contexts[:4] if tight else candidate_contexts
    compact_contexts = [
        _compact_entity_context_for_comparison(context, tight=tight)
    ...
    return json.dumps(
        {
            "extracted_entity": extracted_entity,
            "extracted_entity_markdown": focused_markdown[: (2000 if tight else 6000)],
            "candidate_contexts": compact_contexts,
        },
        ensure_ascii=False,
    )
