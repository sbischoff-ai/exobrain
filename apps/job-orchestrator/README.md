...
- If `GetEdgeExtractionSchemaContext` returns an empty response, step 7 now proceeds with an empty `entity_types` context and logs a warning so relationship matching can continue with reduced schema guidance.
