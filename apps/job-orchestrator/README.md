...
- If `knowledge.update` step 7 fails with `GetEdgeExtractionSchemaContextRequest has no "first_entity_type" field`, sync/regenerate Python gRPC protobufs from `apps/knowledge-interface/proto/knowledge.proto` so worker and knowledge-interface contracts match.
