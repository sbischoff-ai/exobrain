...
- If `knowledge.update` step 5 or 6 fails with OpenAI `400 Bad Request`, verify the agent `response_format` uses a direct Pydantic model or inner JSON schema object (not an OpenAI `{"type":"json_schema","json_schema":...}` envelope) on the native `/internal/chat/messages` contract.
