...
class FakeChatAgent:
    ...
    async def astream(self, message: str, conversation_id: str, *, access_token: str | None = None, session_id: str | None = None) -> AsyncIterator[dict[str, object]]:
        self.calls.append((message, conversation_id, access_token, session_id))
...
async def test_chat_service_stream_events_forwards_conversation_context(fake_database_service, fake_journal_cache, test_settings) -> None:
    ...
    assert agent.calls == [("Prompt A", "conv-1", None, None)]
...
async def test_chat_service_stream_journal_message_persists_user_and_assistant(fake_database_service, fake_journal_cache, test_settings) -> None:
    ...
    assert agent.calls == [("hello", "conv-1", None, None)]
