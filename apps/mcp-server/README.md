...
- one generic success envelope for all tools (built-in and dynamically registered)
  ...
  - `get_entity_context` maps `related_entities` to resolve-compatible fields (`entity_id`, `name`, `aliases`, `entity_type`, `description`) using KI `GetSchema` (blank optional `universe_id`) for type-name lookup with documented fallbacks when schema or descriptions are missing.
